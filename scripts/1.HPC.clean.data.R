...
species.edge.clean <- species.first.clean %>%
  filter(
  ...
  mutate(
    species_range_edge_clean_area_sqkm = units::set_units(st_area(.), km^2) %>%
      as.numeric()
  )

species.edge.clean.not <- species.first.clean %>%
  filter(
    scientific_name %in% c(
      "Fregetta grallaria grallaria",
      "Diomedea antipodensis gibsoni",
...
species.second.clean <- species.edge.clean %>%
  bind_rows(species.edge.clean.not) %T>%
  st_write(
