...
#### Demography, population: Import/clean ####
...
spec.per.elect <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.per.elect.gpkg")
spec.per.elect <- spec.per.elect %>%
  inner_join(demo) %>%
  inner_join(pop) %>%
  relocate(Elect_div, Demographic_class, Electors, total_unique_spec,
           elect_area_sqkm, species_per_sqkm, geom) %T>%
  st_write(dsn = "analysed_data/spatial_ops_output/spec.per.elect.gpkg",
         layer = 'spec.per.elect', append = FALSE) %>%
  st_set_geometry(NULL) %T>%
  write.csv("analysed_data/spatial_ops_output/spec.per.elect.csv")

#### spec.per.elect - nested no. of species per electorate ####

spec.per.elect.nest <- fromJSON("analysed_data/21-11-14_HPC_spatial_ops_output/spec.per.elect.nest.json")
...
spec.eighty.elect <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.eighty.elect.gpkg")

spec.eighty.elect.aus <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.eighty.elect.aus.gpkg")


#### spec.eighty.elect - species range >80% in each electorate ####

spec.endemic.elect <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.endemic.elect.gpkg")

spec.endemic.elect.aus <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.endemic.elect.aus.gpkg")
...
elect.spec.cover.status <- fromJSON("analysed_data/21-11-14_HPC_spatial_ops_output/elect.spec.cover.status.json")
write.csv(elect.spec.cover.status,
          "analysed_data/spatial_ops_output/elect.spec.cover.status.csv")

#### spec.eighty.outside.elect ####

spec.outside.elect <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.outside.elect.gpkg")

spec.outside.elect.logical <- st_read(dsn = "analysed_data/21-11-14_HPC_spatial_ops_output/spec.outside.elect.logical.gpkg")
