...
species <- st_simplify(species,
    dTolerance = 30000) %>% # units of metres, this NULLs geoms < the dTolerance
  st_make_valid() %>%
  filter(!is.na(st_dimension(Shape))) %>%
  slice_sample(n = 100)
elect <- elect %>%
  filter(st_geometry_type(.) %in% c("POLYGON", "MULTIPOLYGON")) %>% # chops geoms rather than casts them
...
spec.per.elect <- elect %>%
  ...
  relocate(Elect_div, Demographic_class, Electors, total_unique_spec,
           elect_area_sqkm, species_per_sqkm, geometry)

st_write(spec.per.elect,
  dsn = "analysed_data/toy_spatial_ops_output/spec.per.elect.gpkg",
  layer = 'spec.per.elect', append = FALSE)

spec.per.elect.no.geom <- spec.per.elect %>%
  st_set_geometry(NULL)

write_json(spec.per.elect.no.geom,
  path = "analysed_data/toy_spatial_ops_output/spec.per.elect.json")
...
spec.per.elect.nest <- elect %>%
  ...
  write_json(path = "analysed_data/toy_spatial_ops_output/spec.per.elect.nest.json")
...
spec.eighty.elect <- spec.range.elect %>%
  ...
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.eighty.elect.gpkg",
         layer = 'spec.eighty.elect', append = FALSE)

spec.eighty.elect.aus <- st_intersection(aus, spec.eighty.elect) %>%
  st_make_valid() %T>%
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.eighty.elect.aus.gpkg",
...
spec.endemic.elect <- spec.range.elect %>%
  ...
  group_by(Elect_div) %>%
  summarise(total_unique_spec = n_distinct(SCIENTIFIC_NAME)) %T>%
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.endemic.elect.gpkg",
         layer = 'spec.endemic.elect', append = FALSE)

spec.endemic.elect.aus <- st_intersection(aus, spec.endemic.elect) %>%
  st_make_valid() %T>%
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.endemic.elect.aus.gpkg",
...
elect.spec.cover.status <- species %>%
  ...
  as.data.frame() %>%
  select(-"geometry") %T>%
  write_json(path = "analysed_data/toy_spatial_ops_output/elect.spec.cover.status.json")
...
spec.eighty.outside.elect <- st_difference(species.area, elect.union) %>%
  st_make_valid() %>%
  ...
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.eighty.outside.elect.gpkg",
         layer = 'spec.eighty.outside.elect', append = FALSE)
...
spec.outside.elect.logical <- species[outside, ] %T>%
  st_write(dsn = "analysed_data/toy_spatial_ops_output/spec.outside.elect.logical.gpkg",
